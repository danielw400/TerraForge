    ...
    public static class SimplexNoise
    {
        public static class Noise
        {
            ...
            public static float CalcPixel3D(float x, float y, float z, float scale)
            {
                ...
                var ny = y * scale;
                var nz = z * scale;
                var xy = PerlinNoise(nx, ny);
                var yz = PerlinNoise(ny, nz);
                var zx = PerlinNoise(nz, nx);
                return (xy + yz + zx) / 3f;
            }

            private static float PerlinNoise(float x, float y)
            {
                var xi = (int)System.Math.Floor(x) & 255;
                var yi = (int)System.Math.Floor(y) & 255;
                var xf = x - System.MathF.Floor(x);
                var yf = y - System.MathF.Floor(y);
                var u = Fade(xf);
                var v = Fade(yf);
